...
exports.getCarLoanList = async ({   agentId, 
    ...
    try {
      ...
      return {
        loanList: dataResult && dataResult.recordset ? dataResult.recordset : [],
        pagination: {
          currentPage: page,
          pageSize: pageSize,
  ...
  // 차량 대출 정보 조회
  exports.getAgentLoanCorpList = async ({ agentId }) => {
    try {
      const request = pool.request();
      request.input("AGENT_ID", sql.VarChar, agentId);
      const query = `SELECT LOAN_CORP_CD, 
                     ...
                     ORDER BY LOAN_CORP_CD;`;

      const result = await request.query(query);
      return result.recordset;
    } catch (err) {
      console.error("Error fetching agent loan corp list:", err);
      throw err;
    }
  };
  ...
  */
  ...
  try {
    ...
    const query1 = `INSERT INTO dbo.CJB_AGENT_LOAN_CORP (
      ...
      console.error("Error inserting agent loan corp:", err);
      throw err;
    }
  };
...
exports.updateAgentLoanCorp = async ({ 
  ...
  try {
    ...
    const query1 = `
      UPDATE dbo.CJB_AGENT_LOAN_CORP  
...
, usrId                        // 사용자 ID
  ...
  try {
    ...
    if (totalCount > 0) {
      throw new Error("Cannot update loan that has interest payments");
    }
    ...
    const delLoanAmt = `
      UPDATE dbo.CJB_AGENT_LOAN_CORP
         SET TOT_LOAN_AMT = TOT_LOAN_AMT - ISNULL((SELECT ISNULL(LOAN_AMT, 0)
                                                     FROM dbo.CJB_CAR_LOAN 
    ...
    request.input("NEW_LOAN_ID", sql.VarChar, newLoanIdValue);    // 대출 ID
    // 재고 금융 등록
    const query1 = `INSERT INTO dbo.CJB_CAR_LOAN (
                      LOAN_ID
                    , CAR_REG_ID
  ...
  `;
    ...
    console.error("Error updating car loan 3:", err);
    throw err;
  }
};
...
exports.deleteAgentLoanCorp = async ({agentId, loan_corp_cd, flag_type}) => {
  try {
    ...
    /* 삭제 처리전 체크 사항
     * 1. TOT_LOAN_AMT가 0이 아닌 경우 삭제 못함.
     */
    ...
    if(totalCount > 0) {
      console.error("Error deleting agent loan corp:", err);
      throw err;
    }
    else {
      query = `DELETE dbo.CJB_AGENT_LOAN_CORP
                 WHERE AGENT_ID = @AGENT_ID
                   AND LOAN_CORP_CD = @LOAN_CORP_CD
          `;
...
// 차량 대출 정보 조회
const getCarLoanCorpLmtAmt = async ({ agentId, loanCorpCd }) => {
  try {
    const request = pool.request();
