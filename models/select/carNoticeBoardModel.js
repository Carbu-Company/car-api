...
*/
    ...
    return {
      ntclist: dataResult && dataResult.recordset ? dataResult.recordset : [],
      pagination: {
        currentPage: page,
        pageSize: pageSize,
...
// 공지 조회
exports.getNoticeInfo = async ({ ntcNo }) => {
  try {
    const request = pool.request();
...
// 공지 삭제
exports.deleteNotice = async ({ ntcNo }) => {
  try {
    const request = pool.request();
    request.input("NTC_NO", sql.Int, ntcNo);
    ...
    console.error("Error deleting notice board detail:", err);
    throw err;
  }
}